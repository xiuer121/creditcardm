...
abstract public class BaseService<T> {
	...
	public View<T> getView(View<T> view) {
		...
		//分页结果集
		List<T> records=getDAO().getListByPage(view);
	 ...
	 * @功能 分页(带查詢)
	 */
	public View<T> getView(String where,View<T> view,Object[] queryParams) {
		...
		//分页结果集 		
		List<T> records=getDAO().getListByPage(view,where,queryParams);
		view.setRecords(records);
